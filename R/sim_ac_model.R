sim_ac_model <- function(Smax, I, E, V1, V2, time, plot_SI = FALSE, plot_SA = FALSE, plot_SC = FALSE, plot_ST = FALSE, plot_ALL = FALSE) {
  ...
  SI_pred <- (I * Smax) / (I + E + V1 + V2) * (1 - exp(-(I + E + V1 + V2) * time))
  ...
  SA_pred <- (V1 * I * Smax / (I + E + V1 + V2)) * ((1 / (E + V2)) * (1 - exp(-(E + V2) * time)) + (1 / (I + V1)) * (exp(-(I + E + V1 + V2) * time) - exp(-(E + V2) * time)))
  ...
  SC_pred <- (V2 * I * Smax / (I + E + V1 + V2)) * ((2 / (E - V2)) * (1 + V1 / (E + V2)) * (1 - exp((V2 - E) * time))
    + (2 / (2 * V2 + I + V1)) * (1 - V1 / (I + V1)) * (exp((V2 - E) * time) - exp(-(I + E + V1 + V2) * time))
    + V1 / V2 * (1 / (E + V1) + 1 / (V1 + I)) * (exp(-(V2 + E) * time) - exp((V2 - E) * time)))
