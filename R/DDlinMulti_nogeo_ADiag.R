...
.createModel_DDlin_multi <- function(tree,r.object){    

        comment <- "Diversity dependent linear model with two slope regimes."
        paramsNames <- c("m0", "logsigma0", "r1", "r2")
        params0 <- c(0,log(1),0,0)


        periodizing <- periodizeOneTree(tree,r.object) 
        ...
        aAGamma <- function(i, params){
        	...
        	R1.mat <- outer(rmat[1,],rmat[1,])
			#diag(R1.mat)<-1
			R2.mat <- outer(rmat[2,],rmat[2,])
			#diag(R2.mat)<-1
			nij1<-colSums(R1.mat)
			nij2<-colSums(R2.mat)
        ...
        constraints <- function(params) return(((exp(params[2])^2)+(params[3]*max(vapply(r.object$S.matrix,function(x)rowSums(x)[1],1))) > 0) &&((exp(params[2])^2)+(params[4]*max(vapply(r.object$S.matrix,function(x)rowSums(x)[2],1))) > 0))
...
periodizeOneTree <- function(tree,r.object){
