sim_ac_model <- function(Smax, I, E, V1, V2, kappa, epsilon, K, time, plot_SI = FALSE, plot_SA = FALSE, plot_SC = FALSE, plot_ST = FALSE, plot_ALL = FALSE) {
  ...
  SI_pred <- (I * Smax) / (I + E * K^kappa + V1 + V2 * K^epsilon) * (1 - exp(-(I + E * K^kappa + V1 + V2 * K^epsilon) * time))
  ...
  SA_pred <- (V1 * I * Smax / (I + E * K^kappa + V1 + V2 * K^epsilon)) * ((1 / (E * K^kappa + V2 * K^epsilon)) * (1 - exp(-(E * K^kappa + V2 * K^epsilon) * time)) + (1 / (I + V1)) * (exp(-(I + E * K^kappa + V1 + V2 * K^epsilon) * time) - exp(-(E * K^kappa + V2 * K^epsilon) * time)))
  ...
  SC_pred <-  (V2 * K^epsilon * I * Smax / (I + E * K^kappa + V1 + V2 * K^epsilon)) * ((2 / (E * K^kappa - V2 * K^epsilon)) * (1 + V1 / (E * K^kappa + V2 * K^epsilon)) * (1 - exp((V2 * K^epsilon - E * K^kappa) * time))
                                                                                       + (2 / (2 * V2 * K^epsilon + I + V1)) * (1 - V1 / (I + V1)) * (exp((V2 * K^epsilon - E * K^kappa) * time) - exp(-(I + E * K^kappa + V1 + V2 * K^epsilon) * time))
                                                                                       + (V1) / (V2 * K^epsilon) * (1 / (E * K^kappa + V1) + 1 / (V1 + I)) * (exp(-(V2 * K^epsilon + E * K^kappa) * time) - exp((V2 * K^epsilon - E * K^kappa) * time)))
