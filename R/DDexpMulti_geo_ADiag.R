...
.createModel_DDexp_multi_geo <- function(tree,geo.object,r.object){    

        comment <- "Diversity dependent exponential model with biogeography and two slope regimes."
        paramsNames <- c("m0", "logsigma0", "r1", "r2")
        params0 <- c(0,log(1),-0.1,-0.1)
        ...
        aAGamma <- function(i, params){
            ...
            matrixGamma <- function(t) return(exp(params[2])*1/colSums(rmat)*(exp(params[3]/2*nij1)*diag(rmat[1,])+exp(params[4]/2*nij2)*diag(rmat[2,])))
