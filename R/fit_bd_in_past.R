.Psi_in_past<-function(s,t,f.lamb,f.mu,desc,tot_desc,cst.lamb=FALSE,cst.mu=FALSE,expo.lamb=FALSE,expo.mu=FALSE,dt=0){
  f<-desc/tot_desc
...
.Phi_in_past<-function(t,f.lamb,f.mu,desc,tot_desc,cst.lamb=FALSE,cst.mu=FALSE,expo.lamb=FALSE,expo.mu=FALSE,dt=0){
  f<-desc/tot_desc	
  if ((cst.lamb==TRUE) & (cst.mu==TRUE))
  {
    lamb <- f.lamb(0)
...
likelihood_bd_in_past<- function(phylo,tot_time,time_stop,f.lamb,f.mu,desc,tot_desc,cst.lamb=FALSE,cst.mu=FALSE,expo.lamb=FALSE,expo.mu=FALSE,dt=0,cond="crown")
{
  ...
  nbtips <- Ntip(phylo)
  log_indLikelihood <- c()
  #from_past <- time_stop+cbind(phylo$edge,node.age(phylo)$ages)
  ...
  for (j in 1:(nbtips-1)){
    ...
    tj <- age-ages[edges[1,1],2] + time_stop
    
    Psi_timevar_errap_tj <- .Psi_in_past(time_stop,tj,f.lamb,f.mu,desc,tot_desc,cst.lamb=cst.lamb,cst.mu=cst.mu,expo.lamb=expo.lamb,expo.mu=expo.mu,dt=dt)
    
    log_lik_tj <- log(f.lamb(tj)) + log(Psi_timevar_errap_tj)
    log_indLikelihood <- c(log_indLikelihood,log_lik_tj)
  }
  log_indLikelihood <- c(log_indLikelihood,log(.Psi_in_past(time_stop,tot_time,f.lamb,f.mu,desc,tot_desc,cst.lamb=cst.lamb,cst.mu=cst.mu,expo.lamb=expo.lamb,expo.mu=expo.mu,dt=dt)))
  
  log_data_lik <- sum(log_indLikelihood)+desc*log(desc/tot_desc)
  ...
  {
    Phi <- .Phi_in_past(tot_time,f.lamb,f.mu,desc,tot_desc,cst.lamb=cst.lamb,cst.mu=cst.mu,expo.lamb=expo.lamb,expo.mu=expo.mu,dt=dt)
...
fit_bd_in_past<-function (phylo,tot_time,time_stop,f.lamb,f.mu,desc,tot_desc,lamb_par, mu_par, meth = "Nelder-Mead", cst.lamb = FALSE, cst.mu = FALSE, expo.lamb = FALSE, 
    expo.mu = FALSE, fix.mu = FALSE, dt = 0, cond = "crown") 
{
    ...
    if (fix.mu == FALSE) {
        ...
        optimLH <- function(init) {
            ...
            LH <- likelihood_bd_in_past(phylo,tot_time,time_stop,f.lamb.par,f.mu.par,desc,tot_desc,cst.lamb = cst.lamb, cst.mu = TRUE,expo.lamb = expo.lamb, dt = dt, cond = cond)
            return(-LH)
        }
        temp <- suppressWarnings(optim(init, optimLH, method = meth))
        lamb.par <- temp$par[1:length(lamb_par)]
...
fit_env_in_past_old <- function(phylo,env_data,tot_time,time_stop,f.lamb,f.mu,desc,tot_desc,lamb_par, mu_par, meth = "Nelder-Mead", cst.lamb = FALSE, cst.mu = FALSE, expo.lamb = FALSE,expo.mu = FALSE, fix.mu = FALSE, dt = 0, df=NULL , cond = "crown") {
  ...
  res <- fit_bd_in_past(phylo=phylo, tot_time=tot_time,time_stop=time_stop, f.lamb=f.lamb.env, f.mu=f.mu.env, lamb_par=lamb_par,mu_par=mu_par, desc=desc, tot_desc=tot_desc, meth, cst.lamb=cst.lamb, cst.mu=cst.mu, expo.lamb=expo.lamb, expo.mu=expo.mu, fix.mu=fix.mu, dt=dt, cond=cond)
                     ...
                     expo.mu=FALSE, fix.mu=FALSE, dt=0, cond="crown")
{
  ...
  res <- fit_bd_in_past(phylo, tot_time=tot_time, time_stop=time_stop, f.lamb=f.lamb.env, f.mu=f.mu.env, lamb_par=lamb_par, mu_par=mu_par,
                        desc=desc, tot_desc=tot_desc, meth=meth, cst.lamb=cst.lamb, cst.mu=cst.mu, expo.lamb=expo.lamb, expo.mu=expo.mu, fix.mu=fix.mu, dt=dt, cond=cond)
  res$model <- "environmental birth death"
