...
likelihood_bd_in_past<- function(phylo,tot_time,time_stop,f.lamb,f.mu,desc,tot_desc,cst.lamb=FALSE,cst.mu=FALSE,expo.lamb=FALSE,expo.mu=FALSE,dt=0,cond="crown")
{
  ...
  #from_past <- cbind(phylo$edge, time_stop + node.age(phylo)$ages) # test
  ...
  for (j in 1:(nbtips-1))
  {
    ...
    Psi_timevar_errap_tj <- .Psi_in_past(0,tj,f.lamb,f.mu,desc,tot_desc,cst.lamb=cst.lamb,cst.mu=cst.mu,expo.lamb=expo.lamb,expo.mu=expo.mu,dt=dt)
    
    log_lik_tj <- log(f.lamb(tj)) + log(Psi_timevar_errap_tj)
    log_indLikelihood <- c(log_indLikelihood,log_lik_tj)
  }
  log_indLikelihood <- c(log_indLikelihood,log(.Psi_in_past(0,tot_time,f.lamb,f.mu,desc,tot_desc,cst.lamb=cst.lamb,cst.mu=cst.mu,expo.lamb=expo.lamb,expo.mu=expo.mu,dt=dt)))
...
fit_env_in_past <- function(phylo,env_data,tot_time,time_stop,f.lamb,f.mu,desc,tot_desc,lamb_par, mu_par, meth = "Nelder-Mead", cst.lamb = FALSE, cst.mu = FALSE, expo.lamb = FALSE,expo.mu = FALSE, fix.mu = FALSE, dt = 0, df=NULL , cond = "crown") {
  if (is.null(df)) {
