...
.createModel_DDlin_geo_ME <- function(tree,geo.object){
    

        comment <- "Diversity dependent linear model with biogeography\n Implemented with measurement error."
        paramsNames <- c("m0","logsigma0", "r","lognuisance")
        params0 <- c(0,log(1),-0.1,log(1))
        ...
        constraints <- function(params) return((exp(params[2])^2)+(params[3]*max(vapply(geo.object$geography.object,function(x)max(rowSums(x)),1))) > 0)
        
        
        model <- new(Class="PhenotypicADiag", name="DDlin_geo", period=periodizing$periods, aAGamma=aAGamma, numbersCopy=eventEndOfPeriods$copy, numbersPaste=eventEndOfPeriods$paste, initialCondition=initialCondition, paramsNames=paramsNames, constraints=constraints, params0=params0, tipLabels=eventEndOfPeriods$labeling,  comment=comment)
