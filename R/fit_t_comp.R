...
if(is.null(error)){
	...
	if(is.null(geography.object) & !is.null(regime.map)){ #two-slope version for sympatric clades
		...
		if(model=="MC"){
			...
			z0<-opt$inferredParams[1]
			eval(parse(text=paste0("results<-list(LH = ",-opt$value,", aic = ",(2*3 - 2*(-opt$value)),", aicc = ",(2*3 - 2*(-opt$value))+((2*3*(3+1))/(length(phylo$tip.label)-3-1)),", free.parameters = 3, sig2 = ",as.numeric(sig2),", S1 (",SMatrix$S1,") = ",as.numeric(S1),", S2 (",SMatrix$S2,") = ",as.numeric(S2),", z0 = ",as.numeric(z0),", convergence = ",opt$convergence,")")))
			return(results)
			}
		if(model=="DDexp"){
			...
			z0<-opt$inferredParams[1]
			eval(parse(text=paste0("results<-list(LH = ",-opt$value,", aic = ",(2*3 - 2*(-opt$value)),", aicc = ",(2*3 - 2*(-opt$value))+((2*3*(3+1))/(length(phylo$tip.label)-3-1)),", free.parameters = 3, sig2 = ",as.numeric(sig2),", r1 (",SMatrix$S1,") = ",as.numeric(r1),", r2 (",SMatrix$S2,") = ",as.numeric(r2),", z0 = ",as.numeric(z0),", convergence = ",opt$convergence,")")))
			return(results)
			}
		if(model=="DDlin"){
			if(is.null(pars)){pars<-c(log(sqrt(var(data)/max(nodeHeights(phylo)))),0,0)}
			...
			z0<-opt$inferredParams[1]
			eval(parse(text=paste0("results<-list(LH = ",-opt$value,", aic = ",(2*3 - 2*(-opt$value)),", aicc = ",(2*3 - 2*(-opt$value))+((2*3*(3+1))/(length(phylo$tip.label)-3-1)),", free.parameters = 3, sig2 = ",as.numeric(sig2),", b1 (",SMatrix$S1,") = ",as.numeric(b1),", b2 (",SMatrix$S2,") = ",as.numeric(b2),", z0 = ",as.numeric(z0),", convergence = ",opt$convergence,")")))
			return(results)
			}
	}
...
if(is.null(geography.object) & !is.null(regime.map)){ #multi-slope version for sympatric clades (i.e., no biogeography)
	...
	if(model=="MC"){
		...
		z0<-opt$inferredParams[1]
		eval(parse(text=paste0("results<-list(LH = ",-opt$value,", aic = ",(2*3 - 2*(-opt$value)),", aicc = ",(2*3 - 2*(-opt$value))+((2*3*(3+1))/(length(phylo$tip.label)-3-1)),", free.parameters = 3, sig2 = ",as.numeric(sig2),", S1 (",SMatrix$S1,") = ",as.numeric(S1),", S2 (",SMatrix$S2,") = ",as.numeric(S2),", nuisance = ",as.numeric(nuisance),", z0 = ",as.numeric(z0),", convergence = ",opt$convergence,")")))
		return(results)
		}
	if(model=="DDexp"){
		...
		nuisance<-exp(opt$inferredParams[5])
		z0<-opt$inferredParams[1]
		eval(parse(text=paste0("results<-list(LH = ",-opt$value,", aic = ",(2*3 - 2*(-opt$value)),", aicc = ",(2*3 - 2*(-opt$value))+((2*3*(3+1))/(length(phylo$tip.label)-3-1)),", free.parameters = 3, sig2 = ",as.numeric(sig2),", r1 (",SMatrix$S1,") = ",as.numeric(r1),", r2 (",SMatrix$S2,") = ",as.numeric(r2),", nuisance = ",as.numeric(nuisance),", z0 = ",as.numeric(z0),", convergence = ",opt$convergence,")")))
		return(results)
		}
	if(model=="DDlin"){
		if(is.null(pars)){pars<-c(log(0.95*sqrt(var(data)/max(nodeHeights(phylo)))),0,0,log(0.05*sqrt(var(data)/max(nodeHeights(phylo)))))}
		...
		nuisance<-exp(opt$inferredParams[5])
		z0<-opt$inferredParams[1]
		eval(parse(text=paste0("results<-list(LH = ",-opt$value,", aic = ",(2*3 - 2*(-opt$value)),", aicc = ",(2*3 - 2*(-opt$value))+((2*3*(3+1))/(length(phylo$tip.label)-3-1)),", free.parameters = 3, sig2 = ",as.numeric(sig2),", b1 (",SMatrix$S1,") = ",as.numeric(b1),", b2 (",SMatrix$S2,") = ",as.numeric(b2),", nuisance = ",as.numeric(nuisance),", z0 = ",as.numeric(z0),", convergence = ",opt$convergence,")")))
		return(results)
		}
}
