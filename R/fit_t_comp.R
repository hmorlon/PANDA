fit_t_comp<-function(phylo,data,model=c("MC","DDexp","DDlin"),geography.object=NULL,method="Nelder-Mead"){
...
if(is.null(geography.object)){
	if(model=="MC"){
		opt<-optim(par=c(0.5,-0.45),likelihood_t_MC,phylo=phylo,data=data,method=method)
	...
	if(model=="DDexp"){
		opt<-optim(par=c(var(data)/max(nodeHeights(phylo)),0),likelihood_t_DD,phylo=phylo,data=data,model="DDexp",method=method)
	...
	if(model=="DDlin"){
		opt<-optim(par=c(var(data)/max(nodeHeights(phylo)),0),likelihood_t_DD,phylo=phylo,data=data,model="DDlin",method=method)
...
#check to make sure length matches length of nodeDiff
	...
	if(model=="MC"){
		opt<-optim(par=c(0.5,-0.45),likelihood_t_MC_geog,phylo=phylo,geography.object=geography.object,data=data,method=method)
		results<-list(lnL = -opt$value, sig2 = abs(opt$par[1]), S = opt$par[2], aic = (2*3 - 2*(-opt$value)), aicc = (2*3 - 2*(-opt$value))+((2*3*(3+1))/(length(phylo$tip.label)-3-1)), free.paramaters = 3, convergence = opt$convergence)
		return(results)
		}
	if(model=="DDexp"){
		opt<-optim(par=c(var(data)/max(nodeHeights(phylo)),0),likelihood_t_DD_geog,phylo=phylo,geography.object=geography.object,data=data,model="DDexp",method=method)
	...
	if(model=="DDlin"){
		opt<-optim(par=c(var(data)/max(nodeHeights(phylo)),0),likelihood_t_DD_geog,phylo=phylo,geography.object=geography.object,data=data,model="DDlin",method=method)
