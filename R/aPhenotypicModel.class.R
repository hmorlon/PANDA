...
setMethod(
    ...
    definition=function(x, ...){
        message("****************************************************************\n")
        message("*** Object of Class PhenotypicModel *** \n")
        message("*** Name of the model : ")
        print(x@name)
        message("*** Parameters of the model : ")
        print(x@paramsNames)
        message("*** Description : ")
        message(x@comment)
        message(paste("\n*** Epochs : the model is cut into ", length(x@period), " parts. \n"))
        print(x@period)
        message("*** Lineages branching (to be copied at the end of the corresponding period) :\n")
        print(x@numbersCopy)
        message("*** Positions of the new trait at the end of each period :\n")
        print(x@numbersPaste)
        message("*** Initial condition :\n")
        print(x@initialCondition)
        message("*** Vectors a_i, A_i, Gamma_i on each period i : \n")
        print(x@aAGamma)
        message("*** Constraints on the parameters : \n")
        print(x@constraints)
        message("*** Defaut parameter values : ")
        print(x@params0)
        message("*** Tip labels : \n")
        print(x@tipLabels)
        message("*** Tip labels for simulations : \n")
...
setMethod(
    ...
    definition=function(object){
        ...
        message("For more details on the model, call : print(PhenotypicModel)\n")
        message("****************************************************************\n")
    }
)
