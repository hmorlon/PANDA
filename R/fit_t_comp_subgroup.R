    ...
    if(is.null(ana.events)){ #subgroup pruning for DD models without biogeography
    	...
    	if(is.null(regime.map)){ #single regime subgroup DD models without biogeography
			...
			if(model=="DDlin"){
				...
				opt<-.fit_t_DD(phylo=full.phylo,data=data, error= error,model="linear",sigma=sigma,beta=beta,subgroup=subgroup,subgroup.map=subgroup.map,method=method,lower=bounds$upper, upper=bounds$upper)
			...
			if(model=="DDlin"){
				...
				eval(parse(text=paste0("results<-list(LH = ",opt$LogLik,", aic = ",opt$AIC,", aicc = ",opt$AICc,", free.parameters =",npar,", sig2 = ",as.numeric(sig2),", b1_",colnames(opt$rates)[2]," = ",as.numeric(b1),", b2_",colnames(opt$rates)[3s]," = ",as.numeric(b2),", z0 = ",as.numeric(z0),", convergence = ",opt$convergence,", nuisance = ",mserr,")")))
				return(results)
			}
