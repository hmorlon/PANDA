           ...
           dt_cst=1e-3, cond="crown")
{
  ...
  if (fix.mu==FALSE)
  {
    ...
    optimLH <- function(init)
    {
      ...
      LH <- likelihood_bd(phylo,tot_time,f.lamb.par,f.mu.par,f,cst.lamb=cst.lamb,cst.mu=cst.mu,expo.lamb=expo.lamb,expo.mu=expo.mu,dt_cst=dt_cst,cond=cond)
      return(-LH)
    }
  ...
  else
  {
    ...
    optimLH <- function(init)
    {
      ...
      f.lamb.par <- function(t){abs(f.lamb(t,lamb_par))}
      f.mu.par <- function(t){abs(f.mu(t,mu_par))}
      LH <- likelihood_bd(phylo,tot_time,f.lamb.par,f.mu.par,f,cst.lamb=cst.lamb,cst.mu=TRUE,expo.lamb=expo.lamb,dt_cst=dt_cst,cond=cond)
      return(-LH)
    }
    temp <- suppressWarnings(optim(init, optimLH, method = meth))
