...
function(name, name_index,simul, mu=1, n=20, seed=1,nb_cores=1,N=300,proportion_variant=0.1,model="uniform",...){
  ...
  if (!exists("provided_tree")){
    ...
    host_tree$tip.label <- sample(paste(rep("H",n),1:n,sep=""))}else{
      if (!is.binary(provided_tree)) stop("Please provide a binary host tree")
      if (!is.rooted(provided_tree)) stop("Please provide a rooted host tree")
      if (!is.ultrametric(provided_tree)) stop("Please provide an ultrametric host tree")
