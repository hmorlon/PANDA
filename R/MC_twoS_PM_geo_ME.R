...
.createModel_MC_twoS_geo_ME <- function(tree,S.object,geo.object){
        ...
        periodizing <- periodizeOneTree_multigeo(tree,S.object) 
        ...
        aAGamma <- function(i, params){
            ...
            matrixGamma <- function(t) return(exp(params[2])*diag(vectorU))
           	R1.mat <- geo.object$geography.object[[i]]*outer(smat[[i]][1,],smat[[i]][1,])
			diag(R1.mat)<-1
			R2.mat <- geo.object$geography.object[[i]]*outer(smat[[i]][2,],smat[[i]][2,])
			diag(R2.mat)<-1
			nij1<-colSums(R1.mat)
			nij2<-colSums(R2.mat)
			...
			#matrixA <-((params[3]*diag(smat[[i]][1,])+params[4]*diag(smat[[i]][2,]))/colSums(smat[[i]])) - (((params[3]/max(sum(smat[[i]][1,]),1)) * outer(smat[[i]][1,],smat[[i]][1,]) + (params[4]/max(sum(smat[[i]][2,]),1)) * outer(smat[[i]][2,],smat[[i]][2,]))/ colSums(smat[[i]]))
           	matrixA<-((params[3]*diag(smat[[i]][1,])+params[4]*diag(smat[[i]][2,]))/colSums(smat[[i]])) - ((((params[3]/nij1) * (geo.object$geography.object[[i]] * outer(smat[[i]][1,],smat[[i]][1,]))) + ((params[4]/nij2) * (geo.object$geography.object[[i]] * outer(smat[[i]][2,],smat[[i]][2,]))))/ colSums(smat[[i]]))
...
periodizeOneTree_multigeo <- function(tree,S.object){
