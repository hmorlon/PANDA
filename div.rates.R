div.rates <- function(phy, shift.estimates.res, combi = 1, part = "backbone",
  ...
  globaldiv <- matrix(NA,length(comb.sub)+length(comb.bck)+1, length(time.seq)) #matrix(NA, Number of clades, Crown age of the whole tree + 1)
  ...
  for(r in 1:length(globalrate)){
    ...
    rate_df <- matrix(NA,2, length(seq(ceiling(tot_time),0,by=-1)))
    row.names(rate_df) <- c("Speciation", "Extinction")

    if (grepl("BCST", model)){
      rate_spec <- rep(rate_data$Lambda[r], length(c(agei, seq(floor(agei),0,by=-1))))
      rate_ext <- rep(NA, length(c(agei, seq(floor(agei),0,by=-1))))
    }
    
    if (grepl("BCST_DCST", model)){
      rate_spec <- rep(rate_data$Lambda[r], length(c(agei, seq(floor(agei),0,by=-1))))
      rate_ext <- rep(rate_data$Mu[r], length(c(agei, seq(floor(agei),0,by=-1))))
    }
    
    if (grepl("BVAR", model)){
      rate_spec <- rate_data$Lambda[r] *exp(rate_data$Alpha[r] * c(agei, seq(floor(agei),0,by=-1)))
      rate_ext <- rep(NA, length(c(agei, seq(floor(agei),0,by=-1))))
    }
    ...
    if (grepl("BCST_DVAR", model)){
      rate_spec <- rep(rate_data$Lambda[r], length(c(agei, seq(floor(agei),0,by=-1))))
      rate_ext <- rate_data$Mu[r] *exp(rate_data$Beta[r] * c(agei, seq(floor(agei),0,by=-1)))
    }
